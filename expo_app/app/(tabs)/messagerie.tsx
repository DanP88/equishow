...
export default function MessagerieScreen() {
  ...
  // Realtime : nouveaux messages → forcer re-render de la conv active
  useRealtimeMessages(
    myId,
    (newMsg) => {
      // Retrouver la conv locale et y ajouter le message reçu
      const conv = messagesStore.list.find(c => c.id === newMsg.conversation_id);
      if (conv && newMsg.sender_id !== myId) {
        ...
        conv.messages = [...conv.messages, {
          ...
          texte: (newMsg as any).content,
          heure,
          ts: new Date(newMsg.created_at).getTime(),
        }];
        conv.dernierMsg = (newMsg as any).content;
        conv.heure = heure;
        conv.unreadBy[myId] = (conv.unreadBy[myId] ?? 0) + 1;
      }
      setTick(t => t + 1);
    },
    (_conv) => {
      // Conv mise à jour (ex: dernier message) → rafraîchir la liste
      refreshConvs();
    }
  );
  ...
  useFocusEffect(useCallback(() => {
    refreshConvs();
    // Pas de polling — les mises à jour arrivent via Realtime (useRealtimeMessages)
  }, []));

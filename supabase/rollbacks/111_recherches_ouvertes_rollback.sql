-- ============================================================================
-- ROLLBACK 111 — RECHERCHES OUVERTES (Transport uniquement)
-- ============================================================================
-- Réversible à 100%. Retire la RPC, les triggers/fonctions de recalcul de
-- couverture, les 4 tables Transport (index + RLS policies droppés en
-- cascade avec chaque table), et les 2 colonnes ajoutées par ALTER sur la
-- vraie table V1 transport_reservations (recherche_id, recherche_reponse_id
-- — jamais écrites hors du parcours recherche, DROP COLUMN sans impact sur
-- le flux V1 direct). Ne touche à AUCUNE autre colonne/table pré-existante
-- (annonces/réservations/payments/escrow intacts). Box/Coach ne sont pas
-- concernés par cette migration (reportés à 112) — rien à en retirer ici.
--

-- ─────────────────────────────────────────────────────────────────────────────
-- 081 — Organisateur V2 LOT 2 (PR2) : clics modules dans le Radar.
--
-- Étend fn_org_concours_radar (CREATE OR REPLACE, ADDITIF sur 080) avec :
--   • module_clicks.{box,transport,coach}  → cta_click action='concours_click_*'
--     attribués au concours via metadata->>'concours_id'
--   • funnel.module_clicks                 → total clics modules (étape funnel)
--
-- CONSERVE : fn_org_owns_concours (42501), masquage RGPD <5, tout 080.
-- ⚠️ Sans historique : les clics ne comptent qu'à partir de l'instrumentation
--    front (PR2). Convention identique à click_ffe.
-- Appliqué prod via `db query -f` + `migration repair --status applied 081`, jamais db push.
-- ─────────────────────────────────────────────────────────────────────────────
...
declare
  ...
  v_clk_box int; v_clk_transport int; v_clk_coach int;
...
begin
  ...
  -- KPI 1 — VISIBILITÉ + CLICS MODULES (events attribués via metadata->>'concours_id')
  select
    ...
    count(*)                       filter (where event_type = 'cta_click' and action = 'concours_click_box'),
    count(*)                       filter (where event_type = 'cta_click' and action = 'concours_click_transport'),
    count(*)                       filter (where event_type = 'cta_click' and action = 'concours_click_coach')
  into v_views, v_unique, v_ffe, v_clk_box, v_clk_transport, v_clk_coach
  ...
  -- KPI 3bis — RÉSERVATIONS PAR MODULE (lignes, hors statuts annulés).
  ...
  -- KPI 5 — CA GÉNÉRÉ (payments succeeded rattachés au concours). amount_* en CENTIMES.
  select
  ...
  return jsonb_build_object(
    ...
    'module_clicks', jsonb_build_object(
      'box', coalesce(v_clk_box,0),
      'transport', coalesce(v_clk_transport,0),
      'coach', coalesce(v_clk_coach,0),
      'total', coalesce(v_clk_box,0) + coalesce(v_clk_transport,0) + coalesce(v_clk_coach,0)
    ),
    ...
    'funnel', jsonb_build_object(
      'views', coalesce(v_views,0),
      'followers', coalesce(v_followers,0),
      'module_clicks', coalesce(v_clk_box,0) + coalesce(v_clk_transport,0) + coalesce(v_clk_coach,0),

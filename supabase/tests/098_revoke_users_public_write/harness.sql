-- Harness 098 — Vérification REVOKE écriture users_public
-- Exécuter après application de la migration :
--   supabase db query --linked -f supabase/tests/098_revoke_users_public_write/harness.sql
--
-- Tous les tests doivent afficher PASS.

\echo '=== H1 : authenticated ne doit plus avoir INSERT sur users_public ==='
SELECT CASE
...
\echo '=== H2 : authenticated ne doit plus avoir UPDATE sur users_public ==='
SELECT CASE
...
\echo '=== H3 : authenticated ne doit plus avoir DELETE sur users_public ==='
SELECT CASE
...
\echo '=== H4 : authenticated doit toujours avoir SELECT sur users_public ==='
SELECT CASE
...
\echo '=== H5 : anon doit toujours avoir SELECT sur users_public ==='
SELECT CASE
...
\echo '=== H6 : grants restants sur users_public (attendu : anon SELECT + authenticated SELECT) ==='
SELECT grantee,

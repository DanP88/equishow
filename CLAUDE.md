...
> MAJ : 2026-07-20 · repo `main` @ `4571b4c` · migrations fichiers → **096** (…085/092/093/094/095/096 appliquées prod). **0 PR ouverte.**
...
- **P3** — push mobile EAS · 23 fichiers parasites untracked · ~30 branches locales mortes · **simplification système de rôles (095 fait ; reste 097/098 non commencées)** : `mig 096` = protection serveur limite d'essai coach (PR #92, prod 2026-07-20) ; `mig 097` = drop `is_admin()` ; `mig 098` = drop tables legacy `profiles`/`roles` — retrait final du legacy au profit de `users.role`/`is_app_admin()` (audit + plan dans `docs/security.md`).

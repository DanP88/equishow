...
> MAJ : 2026-07-29 · repo `main` @ `50d8c27` · migrations fichiers → **102** (…102 appliquées prod). **0 PR ouverte.**
...
🟢 **code prêt** / 🔴 **infra commerciale non prête**. `main` = `origin/main` @ `50d8c27`, **0 PR ouverte**.
...
- **Admin** — dashboards, remboursements/litiges réels, approbation claims, import concours. Admin = `users.role='admin'` (**source de vérité autoritative**). ⚠️ Un système **legacy** parallèle existe (`profiles.role_id` → `roles`), non utilisé par le front ni les Edge Functions ; sécurisé par mig 094 (verrou colonne `role_id` + `search_path`). `is_admin()` **supprimée (mig 101)**. Toute nouvelle policy admin doit s'appuyer sur `users.role='admin'` via `is_app_admin()`.

...
> MAJ : 2026-07-29 · repo `main` @ `efb9146` · migrations fichiers → **100** (…097 appliquées prod · 098/099/100 en cours). **3 PR ouvertes (#94/#95/#96).**
...
- **P3** — push mobile EAS · 23 fichiers parasites untracked · ~30 branches locales mortes · **simplification système de rôles (095 fait ; reste 101/102 non commencées)** : `mig 096` = protection serveur limite d'essai coach (PR #92, prod 2026-07-20) ; `mig 097` = drop `_backup_users_plan_085` ✅ prod (PR #93, 2026-07-29 — alerte RLS Security Advisor fermée) ; `mig 098` = REVOKE écriture `users_public` (PR #94, en cours) ; `mig 099` = REVOKE écriture `coach_stats` (PR #95, en cours) ; `mig 100` = `security_invoker=true` vues analytics (PR #96, en cours) ; `mig 101` = drop `is_admin()` ; `mig 102` = drop tables legacy `profiles`/`roles` — retrait final du legacy au profit de `users.role`/`is_app_admin()` (audit + plan dans `docs/security.md`).
